// To test:
// cppcheck --dump misra-test.c && python ../misra.py -verify misra-test.c.dump

// TODO: #include "path\file.h" // 20.2
// TODO: #include /*abc*/ "file.h" // 20.3
...
void misra_5_2_function_hides_var_31y(void)
{
} // 5.2
void foo(void)
{
...
enum misra_5_3_Enum
{
  misra_5_3_var_hidesenumconst_31x = 2,
  misra_5_3_enum_hidesfunction_31x = 5
};
...
void misra_5_3_enum_hidesfunction_31y(void)
{
} // 5.3
...
#define m2(misra_5_4_param_hides_param__31x,                                  \
           misra_5_4_param_hides_param__31y)                                  \
  1 // 5.4
...
void misra_5_5_functionhides_macro31y(int misra_5_5_param_hides_macro__31y)
{
} // 5.5
...
void misra_8_14(char* restrict str)
{
} // 8.14
...
{
  u16* p16;
  int x = (u8*)p; // 11.5 gcc warning: incompatible pointer to integer
                  // conversion initializing 'int' with an expression of type
                  // 'u8 *' (aka 'unsigned char *') [-Wint-conversion]
  p16 = p;        // 11.5
}
...
{
#ifdef DEBUG
  float x = (float)p; // 11.7 gcc error: pointer cannot be cast to type 'float'
  int y = (int*)f;    // 11.7 gcc error: operand of type 'float' cannot be cast
                      // to a pointer type
#endif
}
...
void misra_11_9()
{
  int* p1 = (5 - 5); // 11.9 gcc warning: expression which evaluates to zero
                     // treated as a null pointer constant of type 'int *'
...
void misra_12_1(int x, int y)
{
  int b, c, d;           // error: Uninitialized variable: b
  int sz = sizeof x + y; // 12.1
  int a = (b * c) + d;
  a = b << c + d; // 12.1 gcc warning: operator '<<' has lower precedence
...
void misra_12_3()
{
  f((1, 2), 3); // TODO gcc warning: implicit declaration of function 'f' is
...
{
  int x = 123456u *
    123456u; // 12.4 gcc warning: unused variable 'x' [-Wunused-variable]
}
...
void misra_13_3(int x, int y)
{
  x = y++; // 13.3
}

void misra_13_4(int x, int y, int z)
{
  if (x != (y = z)) {
  } // 13.4
  else {
...
{
  int x;
  return sizeof(x++); // 13.6  warning: Found calculation inside sizeof().
}
...
void misra_15_6(int x)
{
  ...
  do {
  } while (x < 0); // no-warning
}

void misra_15_7(int x)
{
  if (x != 0) {
  } // no-warning
  if (x != 0) {
...
void misra_16_3(int x)
{
  ...
  switch (x) {
    ...
    case 2:
      a = 1;
    case 3: // 16.3 warning: Variable 'a' is reassigned a value before the old
...
void misra_17_6(int x[static 20])
{
} // 17.6
...
void misra_21_3()
{
  void* p1 = malloc(10);    // 21.3
  void* p2 = calloc(2, 10); // 21.3
  realloc(p2, 10);          // 21.3 error: Memory leak: p2
  free(p1);                 // 21.3
}

void misra_21_7()
{
  const char* str = "123.7";
  atof(str);  // 21.7 warning: Return value of function atof() is not used.
  atoi(str);  // 21.7
  atol(str);  // 21.7
  atoll(str); // 21.7
}
