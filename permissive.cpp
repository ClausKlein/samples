...
//
//
//from http://stackoverflow.com/questions/642229/why-do-i-need-to-use-typedef-typename-in-g-but-not-vs
//
...
struct remove_cv
{
    ...
    //FIXME typedef std::remove_volatile<std::remove_const<T>::type>::type type;
};
...
{
    typename std::map<KEY, VALUE>::const_iterator iter = container.find(key); // error: missing 'typename' prior to dependent type name
...
int f(T const &)
{
    return Test<T>::value;  // during first pass, Test<T>::value is interpreted as a value
}
...
int main()
{
    ...
    //FIXME f(5);           // error: missing 'typename' prior to dependent type name
    //TODO int i = f(5);    // error: missing 'typename' prior to dependent type name
